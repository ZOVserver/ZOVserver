...
public static class EventsManager
{
    ...
    private static TrophySeasonGlobalEvent _currentSeason = new();
    ...
    public static async Task StartConsumerAsync(INatsConnection nats)
    {
        ...
        var seasonSub = nats.SubscribeAsync<TrophySeasonGlobalEvent>("GlobalEvents.TrophySeason");
        ...
        _ = Task.Run(async () =>
        {
            await foreach (var msg in seasonSub)
                if (msg.Data != null)
                    SetCurrentSeason(msg.Data);
        });
    ...
    public static TrophySeasonGlobalEvent GetCurrentSeason()
    {
        return Interlocked.CompareExchange(ref _currentSeason!, null, null);
    }
    ...
    private static void SetCurrentSeason(TrophySeasonGlobalEvent season)
    {
        Interlocked.Exchange(ref _currentSeason, season);
    }
