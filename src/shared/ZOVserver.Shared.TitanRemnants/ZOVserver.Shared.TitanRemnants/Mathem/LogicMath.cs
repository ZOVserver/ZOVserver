namespace ZOVserver.Shared.TitanRemnants.Mathem;

public static unsafe class LogicMath
{
    private static readonly byte[] SqrtTable =
    [
        
    ];

    private static readonly byte[] AtanTable =
    [
        
    ];

    private static readonly int[] SinTable =
    [
        0x0000, 0x0012, 0x0024, 0x0036, 0x0047, 0x0059, 0x006B, 0x007D,
        0x008F, 0x00A0, 0x00B2, 0x00C3, 0x00D5, 0x00E6, 0x00F8, 0x0109,
        0x011A, 0x012B, 0x013C, 0x014D, 0x015E, 0x016F, 0x0180, 0x0190,
        0x01A0, 0x01B1, 0x01C1, 0x01D1, 0x01E1, 0x01F0, 0x0200, 0x020F,
        0x021F, 0x022E, 0x023D, 0x024B, 0x025A, 0x0268, 0x0276, 0x0284,
        0x0292, 0x02A0, 0x02AD, 0x02BA, 0x02C7, 0x02D4, 0x02E1, 0x02ED,
        0x02F9, 0x0305, 0x0310, 0x031C, 0x0327, 0x0332, 0x033C, 0x0347,
        0x0351, 0x035B, 0x0364, 0x036E, 0x0377, 0x0380, 0x0388, 0x0390,
        0x0398, 0x03A0, 0x03A7, 0x03AF, 0x03B5, 0x03BC, 0x03C2, 0x03C8,
        0x03CE, 0x03D3, 0x03D8, 0x03DD, 0x03E2, 0x03E6, 0x03EA, 0x03ED,
        0x03F0, 0x03F3, 0x03F6, 0x03F8, 0x03FA, 0x03FC, 0x03FE, 0x03FF,
        0x03FF, 0x0400, 0x0400
    ];

    public static int Abs(int value)
    {
        return Math.Abs(value);
    }

    public static int Sign(int a1)
    {
        return Math.Sign(a1);
    }

    public static int Min(int valueA, int valueB)
    {
        return Math.Min(valueA, valueB);
    }

    public static int Max(int valueA, int valueB)
    {
        return Math.Max(valueA, valueB);
    }

    public static int Clamp(int clampValue, int minValue, int maxValue)
    {
        return Math.Clamp(clampValue, minValue, maxValue);
    }

    public static int Sin(int angle)
    {
        angle = NormalizeAngle360(angle);

        if (angle < 180)
        {
            if (angle > 90)
                angle = 180 - angle;

            return SinTable[angle];
        }

        angle -= 180;

        if (angle > 90)
            angle = 180 - angle;

        return -SinTable[angle];
    }

    public static int Cos(int angle)
    {
        return Sin(angle + 90);
    }

    public static int GetAngle(int x, int y, int startX, int startY)
    {
        if (startX != 0) x -= startX;
        if (startY != 0) y -= startY;

        var angle = Math.Atan2(y, x) * (180 / Math.PI);
        if (angle < 0) angle += 360;

        return (int)Math.Round(angle);
    }

    public static int GetAngle(int x, int y)
    {
        // ReSharper disable once ConvertIfStatementToSwitchStatement
        if (x == 0 && y == 0)
            return 0;

        if (x > 0 && y >= 0)
        {
            if (y >= x)
                return 90 - AtanTable[(x << 7) / y];

            return AtanTable[(y << 7) / x];
        }

        var num = Abs(x);

        if (x <= 0 && y > 0)
        {
            if (num < y)
                return 90 + AtanTable[(num << 7) / y];

            return 180 - AtanTable[(y << 7) / num];
        }

        var num2 = Abs(y);

        if (x < 0 && y <= 0)
        {
            if (num2 < num)
                return 180 + AtanTable[(num2 << 7) / num];

            if (num2 == 0)
                return 0;

            return 270 - AtanTable[(num << 7) / num2];
        }

        if (num < num2)
            return 270 + AtanTable[(num << 7) / num2];

        return num == 0 ? 0 : NormalizeAngle360(360 - AtanTable[(num2 << 7) / num]);
    }

    public static int GetAngleBetween(int a1, int a2)
    {
        var result = (a1 - a2) % 360 + ((a1 - a2) % 360 < 0 ? 360 : 0);

        if (result > 179)
            result -= 360;

        if (result < 0)
            return -result;

        return result;
    }

    public static int GetRotatedX(int x, int y, int angle)
    {
        int v3; // r3
        int v4; // r5
        int v5; // r4
        int v6; // r0
        int v7; // r2
        int v8; // r2
        int v9; // r3

        v3 = (angle + 90) % 360 + ((angle + 90) % 360 < 0 ? 0x168 : 0);

        if (v3 > 179)
        {
            v5 = v3 - 180;

            if (v3 - 180 > 90)
                v5 = 360 - v3;

            v4 = -SinTable[v5];
        }
        else
        {
            if (v3 > 90)
                v3 = 180 - v3;

            v4 = SinTable[v3];
        }

        v6 = v4 * x;
        v7 = angle % 360 + (angle % 360 < 0 ? 0x168 : 0);

        if (v7 > 179)
        {
            v9 = v7 - 180;

            if (v7 - 180 > 90)
                v9 = 360 - v7;

            v8 = -SinTable[v9];
        }
        else
        {
            if (v7 > 90)
                v7 = 180 - v7;

            v8 = SinTable[v7];
        }

        return (v6 - v8 * y) / 1024;
    }

    public static int GetRotatedY(int x, int y, int angle)
    {
        int v3; // r3
        int v4; // r5
        int v5; // r4
        int v6; // r0
        int v7; // r2
        int v8; // r2
        int v9; // r3

        v3 = angle % 360 + (angle % 360 < 0 ? 0x168 : 0);

        if (v3 > 179)
        {
            v5 = v3 - 180;

            if (v3 - 180 > 90)
                v5 = 360 - v3;

            v4 = -SinTable[v5];
        }
        else
        {
            if (v3 > 90)
                v3 = 180 - v3;

            v4 = SinTable[v3];
        }

        v6 = v4 * x;
        v7 = (angle + 90) % 360 + ((angle + 90) % 360 < 0 ? 0x168 : 0);

        if (v7 > 179)
        {
            v9 = v7 - 180;

            if (v7 - 180 > 90)
                v9 = 360 - v7;

            v8 = -SinTable[v9];
        }
        else
        {
            if (v7 > 90)
                v7 = 180 - v7;

            v8 = SinTable[v7];
        }

        return (v6 + v8 * y) / 1024;
    }

    public static int NormalizeAngle180(int angle)
    {
        angle = NormalizeAngle360(angle);

        if (angle >= 180)
            return angle - 180;

        return angle;
    }

    public static int NormalizeAngle360(int angle)
    {
        angle %= 360;

        if (angle < 0)
            return angle + 360;

        return angle;
    }

    public static int Sqrt(int value)
    {
        if (value < 0) return -1;

        fixed (byte* sqrtTable = SqrtTable)
        {
            if (value >= 0x10000)
            {
                int num;

                if (value >= 0x1000000)
                {
                    if (value >= 0x10000000)
                        num = value >= 0x40000000
                            ? value == 0x7FFFFFFF ? 0xFFFF : sqrtTable[value >> 24] << 8
                            : sqrtTable[value >> 22] << 7;
                    else if (value >= 0x4000000)
                        num = sqrtTable[value >> 20] << 6;
                    else
                        num = sqrtTable[value >> 18] << 5;

                    num = (num + 1 + value / num) >> 1;
                    num = (num + 1 + value / num) >> 1;
                    return num * num <= value ? num : num - 1;
                }

                if (value >= 0x100000)
                    num = value >= 0x400000
                        ? sqrtTable[value >> 16] << 4
                        : sqrtTable[value >> 14] << 3;
                else if (value >= 0x40000)
                    num = sqrtTable[value >> 12] << 2;
                else
                    num = sqrtTable[value >> 10] << 1;

                num = (num + 1 + value / num) >> 1;

                return num * num <= value ? num : num - 1;
            }

            if (value >= 0x100)
            {
                int num;

                if (value >= 0x4000)
                    num = sqrtTable[value >> 8] + 1;
                else if (value >= 0x1000)
                    num = (sqrtTable[value >> 6] >> 1) + 1;
                else if (value >= 0x400)
                    num = (sqrtTable[value >> 4] >> 2) + 1;
                else
                    num = (sqrtTable[value >> 2] >> 3) + 1;

                return num * num <= value ? num : num - 1;
            }

            return sqrtTable[value] >> 4;
        }
    }

    public static int GetRadius(int x, int y)
    {
        x = Abs(x);
        y = Abs(y);

        var maxValue = Max(x, y);
        var minValue = Min(x, y);

        return maxValue + ((53 * minValue) >> 7);
    }

    public static int GetDaysInMonth(int month, int year = -1)
    {
        if (year < 0) year = DateTime.Today.Year;
        return DateTime.DaysInMonth(year, month);
    }
}